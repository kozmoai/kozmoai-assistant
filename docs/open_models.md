...
You can integrate `kozmoai-assistant` with open-source models by leveraging an OpenAI-compatible API.
...
And run `kozmoai-assistant` with the following command:
...
Then you call `kozmoai-assistant` with your service endpoint `--azure https://aoi-resource-name.openai.azure.com` and set your deployment name (which you created in the Azure AI Studio) as the model name (last `kozmoai-assistant` argument).

Example:
`kozmoai-assistant --azure https://myairesource.openai.azure.com ./projects/example/ my-gpt4-project-name`
