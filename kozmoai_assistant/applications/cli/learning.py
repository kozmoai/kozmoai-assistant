"""
The `learning` module is designed to facilitate the collection and storage of user feedback on the outputs generated by the KOZMOAI Assistant tool. It provides mechanisms for obtaining user consent, capturing user reviews, and storing this information for future analysis and enhancement of the tool's performance.
...
Learning : dataclass
    Encapsulates the metadata and feedback collected during a session of using the KOZMOAI Assistant tool, including the prompt, model, temperature, configuration, logs, session identifier, user review, and timestamp.
...
ask_collection_consent() -> bool
    Prompts the user for consent to store their data for the purpose of improving KOZMOAI Assistant.
...
class Learning:
    """
    A dataclass that encapsulates the learning data collected during a KOZMOAI Assistant session.

    Attributes
    ----------
    prompt : str
        A JSON string representing the prompt provided to KOZMOAI Assistant.
    model : str
...
def human_review_input() -> Optional[Review]:
    ...
    print(
        colored(
            "To help kozmoai-assistant learn, please answer 3 questions:", "light_green"
        )
    )
    print()
...
def ask_collection_consent() -> bool:
    """
    Asks the user for their consent to store their data for the purpose of improving the KOZMOAI Assistant tool.
    ...
    """
    answer = input(
        "Is it ok if we store your prompts to help improve KOZMOAI Assistant? (y/n)"
    )
    ...
    if answer.lower() == "y":
        ...
        print(
            colored(
                "No worries! KOZMOAI Assistant will not collect your prompts. ❤️",
                "light_green",
            )
        )
        return False
...
def extract_learning(
    ...
    ----------
    prompt : str
        The initial prompt provided to the KOZMOAI Assistant.
    model : str
        The name of the model used during the session.
    temperature : float
        The temperature setting used for the model's responses.
...
def get_session() -> str:
    ...
    str
        A unique identifier for the user session.
    """
    path = Path(tempfile.gettempdir()) / "kozmoai_assistant_user_id.txt"
