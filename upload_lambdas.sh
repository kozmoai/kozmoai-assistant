...
usage() { echo "Usage: $0 -f <lambda-function-names>  -b <branch name> [-s <bucket name>]" 1>&2; exit 1; }

while getopts ":f:b:s:" arg;
do
  case ${arg} in
    f)
...
then
  ...
  do
    ...
    aws s3 cp ${BUILDS_FOLDER}/${f}.zip s3://${ARTEFACT_BUCKET}/lambdas/${BRANCH_NAME}/${f}.zip
  done
fi
